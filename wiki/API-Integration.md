    ...
    items: [
      {
        ...
        onMenuItemClick: async (menu) => {
          ...
          api.insertText({ text: `@UUID[${actor.uuid}]{Our Party}`, menu });
        },
      },
      {
        key: "the-villain",
        ...
        onMenuItemClick: async (menu) => {
          const { uuid, name } = await fromUuid(
            "Compendium.dnd5e.monsters.Actor.bfh29vIEoGzI240e",
          );
          api.insertText({ text: `@UUID[${uuid}]{${name}}`, menu });
        },
      },
    ],
  };
...
![An open journal showing the custom menu items](<./images/example-custom-menu-items.webp>)
