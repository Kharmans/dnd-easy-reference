...
console.log("D&D Easy Reference | Reforging Symlinks");
...
if (fs.existsSync(symlinkConfigPath)) {
  ...
  if (!!symlinkConfig.dnd5eInstallPath) {
    await linkDnd5eFiles(symlinkConfig.dnd5eInstallPath);
  }
} else {
  console.log(
    `D&D Easy Reference | ${symlinkConfigPath} not found. Create this file in the project root using the example file for guidance.`,
  );
}
...
async function getFoundryFileRoot(foundryInstallPath) {
  try {
    ...
    console.error(`D&D Easy Reference | Error reading ${symlinkConfigPath}: ${err}`);
  }
...
async function linkDnd5eFiles(dnd5eFileRoot) {
  await fs.promises.symlink(
    path.join(dnd5eFileRoot, "."),
    path.join("foundry", "dnd5e"),
  );
}
